...
//add question
if(isset($_SESSION['key'])){
    if(@$_GET['q']== 'addqns' && $_SESSION['key']=='letspractice') {
    $n=@$_GET['n'];
    $eid=@$_GET['eid'];
    $ch=@$_GET['ch'];
...
header("location:teacherdash.php?q=0");
}
    }
//quiz start
if(@$_GET['q']== 'quiz' && @$_GET['step']== 2) {
    ...
    {
    $sn++;
    header("location:StudentDash.php?q=quiz&step=2&eid=$eid&n=$sn&t=$total")or die('Error152');
    }
    ...
    header("location:StudentDash.php?q=result&eid=$eid");
    }
    else
    {
    header("location:StudentDash.php?q=result&eid=$eid");
